...
import { trackCertificateDownload, trackCertificateShare } from '../services/analyticsService'
...
export async function downloadCertificate(): Promise<void> {
  ...
  if (!eligible) {
    showToast({ message: `Complete all ${TOTAL_MODULES} modules to earn your certificate!`, type: 'info' })
    isGenerating = false
    return
  }
...
function showShareModal(name: string): void {
  ...
  // Track social share clicks in GA4
  overlay.querySelector('.share-btn-twitter')?.addEventListener('click', () => {
    trackCertificateShare('twitter')
  })
  overlay.querySelector('.share-btn-linkedin')?.addEventListener('click', () => {
